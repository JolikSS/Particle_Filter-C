...
void predict(Particle* particles, int n, double Q, double smoothed_velocity)
{
    for (int i = 0; i < n; i++)
    {
        // Все частицы используют одну сглаженную скорость фильтра
        particles[i].v = smoothed_velocity;

        // Предсказание позиции: старая позиция + сглаженная скорость + шум процесса
        particles[i].x += smoothed_velocity + rand_normal(0, sqrt(Q));
    }
}
...
void confidence_interval(Particle* particles, int n, double* low, double* high)
{
    ...
    // Для нормального распределения интервал примерно равен mean ± 1.96 * std
    *low = mean - 1.96 * std;
    *high = mean + 1.96 * std;
}
...
{   
    ...
    double smoothed_velocity = 1.0;      // Начальная сглаженная скорость
    double prev_est = estimate_position(particles, N); // Предыдущая оценка фильтра
/* ==================== ОСНОВНОЙ ЦИКЛ ФИЛЬТРАЦИИ ==================== */
    ...
    for (int step = 0; step < STEPS; step++)
    {
        ...
        predict(particles, N, Q, smoothed_velocity);
        ...
        double instant_velocity = est - prev_est;

        // EMA: новая сглаженная скорость = часть новой скорости + часть старой сглаженной скорости
        smoothed_velocity = alpha * instant_velocity + (1.0 - alpha) * smoothed_velocity;
        ...
        printf("Истинное: %.3f | Измерение: %.3f | Оценка: %.3f | EMA velocity: %.3f\n", true_x, z, est, smoothed_velocity);
