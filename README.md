...
Эта визуализация показывает не только итоговую траекторию, но и то, как сам Particle Filter “думает” на каждом шаге симуляции.

На графике используются три оси:

- **Step** — номер шага симуляции;
- **Position** — положение частицы в одномерном пространстве;
- **Normalized Weight** — нормализованный вес частицы от `0` до `1`.

Каждая точка на графике — это отдельная частица в конкретный момент времени.  
Её положение по оси **Position** показывает, где частица предполагает нахождение объекта, а высота по оси **Normalized Weight** показывает, насколько эта частица считается вероятной после сравнения с измерением сенсора.

Цвет точки также отражает вес частицы:
...
На графике также отображаются три линии:

- **True Position** — истинное положение объекта;
- **Measurement** — шумное измерение сенсора;
- **Estimate** — оценка положения, вычисленная Particle Filter.

Линия **Measurement** обычно заметнее отклоняется от истинного положения, потому что содержит случайный шум.  
Линия **Estimate** должна идти ближе к **True Position**, так как фильтр использует множество частиц и их веса, чтобы сгладить шум измерений.

Высокие “пики” на 3D-графике показывают области, где частицы получили большой вес. Это означает, что фильтр считает такие положения более вероятными. Если частицы концентрируются рядом с истинной траекторией, значит фильтр успешно отслеживает объект.
...
- как веса меняются от шага к шагу;
